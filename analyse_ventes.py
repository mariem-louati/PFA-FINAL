...
# ─────────────────────────────────────────────
# 1. Générer ventes2.csv
# ─────────────────────────────────────────────
produits = list(range(101, 121))  # 20 produits
lignes = []
...
with open("ventes2.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["ID", "Prix", "Quantite", "Remise"])
    writer.writerows(lignes)

print("✔ ventes2.csv généré avec succès.\n")

# ─────────────────────────────────────────────
# 2-7. Calculs & export results_final2.csv
# ─────────────────────────────────────────────
TAUX_TVA = 0.20

resultats = []

with open("ventes2.csv", newline="") as f:
...
# 7. Export results_final2.csv
colonnes = ["ID", "Prix", "Quantite", "Remise", "CA_Brut", "CA_Net", "TVA", "CA_TTC"]
with open("results_final2.csv", "w", newline="") as f:
    writer = csv.DictWriter(f, fieldnames=colonnes)
    writer.writeheader()
    writer.writerows(resultats)

print("✔ results_final2.csv exporté avec succès.\n")
...
plt.tight_layout()
plt.savefig("graphiques_ventes2.png", dpi=120)
plt.close()
print("✔ graphiques_ventes2.png généré avec succès.")